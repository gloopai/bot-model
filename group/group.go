...
type GroupConfig struct {
	...
	SensitiveWordConfig *SensitiveWordConfig `json:"sensitive_word_config"` // 敏感词监控配置
}

// 敏感词监控配置
type SensitiveWordConfig struct {
	Words []SensitiveWordRule `json:"words"` // 敏感词规则列表
}

// 敏感词规则
type SensitiveWordRule struct {
	Keyword      string              `json:"keyword"`       // 敏感词
	HandleAction SensitiveWordAction `json:"handle_action"` // 处理方式
	BanDuration  int                 `json:"ban_duration"`  // 封禁时长（秒），仅当处理方式为BanUser时有效
}

// 敏感词处理方式
type SensitiveWordAction int

const (
	SensitiveWordNoAction SensitiveWordAction = iota // 不处理
	SensitiveWordBanUser                             // 封禁用户
)
...
const (
	VerificationNone     VerificationMethod = iota // 无需验证
	VerificationQuestion                           // 答题验证
	VerificationManual                             // 管理员手动审核
	// 可扩展更多方式
)
...
type VerificationConfig struct {
	...
	Method                VerificationMethod // 验证方式
...
type GroupJoinOptions struct {
	...
	VerificationConfig *VerificationConfig // 验证配置（仅Method为GroupJoinVerification时有效）
}
...
type FilterResult struct {
	...
	Reason        string
	SensitiveHit  bool
	SensitiveRule *SensitiveWordRule
}
